...
def translate_data(grants_data: dict):
    trust_dict = {}  # dict of values
    grants_list = []  # list of 3 elements
    for g in grants_data:
        ...
        for c in g.get('contributions'):
            ...
            if profile_id:
                val = [grant_id] + \
                    [c.get('id')] + [c.get('sum_of_each_profiles_contributions')]
...
def get_totals_by_pair(contrib_dict: Dict[int, Dict[int, float]]):
    tot_overlap = {}
...
'''
    ...
    args:
        aggregated contributions by pair nested dict
        -- Total contribution per (grant, user) pair
            {
                grant_id (str): {
                    user_id (str): aggregated_amount (float)
                }
        ...
        pair_totals
            ...
            {user_id (str): {user_id (str): pair_total (float)}}
        trust_dict
        -- Mapping from a user_id to a simple numerical value (given)
            {user_id (str): trust_score (float)}
        v_threshold 
        -- Unverified threshold (constant)
            float
        uv_threshold
        -- Unverified thershold (constant)
            float
        total_pot
            float
    returns:
...
def calculate_clr(aggregated_contributions: Dict[str, Dict[str, float]],
                  pair_totals: Dict[str, Dict[str, float]],
                  trust_dict: Dict[str, float],
                  v_threshold: float,
                  uv_threshold: float,
                  total_pot: float):
    ...
    for proj, contribz in aggregated_contributions.items():
        ...
        for k1, v1 in contribz.items():
            ...
            for k2, v2 in contribz.items():
                if k2 > k1:
                    tot += ((v1 * v2) ** 0.5) / (pair_totals[k1][k2] / (
                        v_threshold * max(trust_dict[k2], trust_dict[k1])) + 1)
    ...
    if bigtot >= total_pot:  # saturation reached
        # print(f'saturation reached. Total Pot: {bigtot}. Normalizing')
...
def run_clr_calcs(grant_contribs_curr, v_threshold, uv_threshold, total_pot):
    ...
    # aggregate data (sum over all contributions)

    curr_agg: Dict[Dict[str, float]] = aggregate_contributions(curr_round)

    # get pair totals
    ptots = get_totals_by_pair(curr_agg)

    # clr calcluation
    totals = calculate_clr(curr_agg, ptots, trust_dict,
                           v_threshold, uv_threshold, total_pot)
...
def fetch_data(clr_round: object, network='mainnet'):
    ...
    contributions = Contribution.objects.prefetch_related('subscription', 'profile_for_clr').filter(
        match=True, created_on__gte=clr_start_date, created_on__lte=clr_end_date, success=True).nocache()
    if subscription_filters:
        contributions = contributions.filter(**subscription_filters)

    grants = Grant.objects.filter(
        network=network, hidden=False, active=True, is_clr_eligible=True, link_to_new_grant=None)
    ...
    if grant_filters:
        ...
        # Collection Filters
        grant_ids = GrantCollection.objects.filter(
            **collection_filters).values_list('grants', flat=True)
...
def populate_data_for_clr(grants, contributions, clr_round) -> List[dict]:
    ...
    for grant in grants:
        ...
        # contributions
        contribs = copy.deepcopy(contributions).filter(subscription__grant_id=grant.id,
                                                       subscription__is_postive_vote=True, created_on__gte=clr_start_date, created_on__lte=clr_end_date)

        # combine
        y: List[Tuple[object, float]] = [(c.identity_identifier(mechanism), c.profile_for_clr.trust_bonus)
                                         for c
                                         in contribs]
        x = set(y)
        contributing_profile_ids = list(x)
        ...
        if len(contributing_profile_ids) > 0:
            for profile_id, trust_bonus in contributing_profile_ids:
                profile_contributions = contribs.filter(
                    profile_for_clr__id=profile_id)
                sum_of_each_profiles_contributions = float(sum(
                    [c.subscription.amount_per_period_usdt * clr_round.contribution_multiplier for c in profile_contributions if c.subscription.amount_per_period_usdt]))
    ...
    return contrib_data_list: List[List[dict]]
...
def predict_clr(save_to_db=False, from_date=None, clr_round=None, network='mainnet', only_grant_pk=None):
    ...
    grant_contributions_curr: List[dict] = populate_data_for_clr(
        grants, contributions, clr_round)
    ...
    for grant in grants:
        ...
        for amount in potential_donations:
            ...
            predicted_clr, grants_clr, _, _ = calculate_clr_for_donation(
                grant,  # (dict variable)
                amount,  # (float variable)
                grant_contributions_curr,  # (list of dicts)
                total_pot,  # (float constant)
                v_threshold,  # (float constant)
                uv_threshold  # (float constant)
            )
            potential_clr.append(predicted_clr)

        if save_to_db:
            _grant = Grant.objects.get(pk=grant.pk)
            _grant.clr_prediction_curve = list(
                zip(potential_donations, potential_clr))
            ...
            can_estimate = True if base or _grant.clr_prediction_curve[1][
                1] or _grant.clr_prediction_curve[2][1] or _grant.clr_prediction_curve[3][1] else False

            if can_estimate:
                _grant.clr_prediction_curve = [
                    [ele[0], ele[1], ele[1] - base] for ele in _grant.clr_prediction_curve]
            else:
                _grant.clr_prediction_curve = [
                    [0.0, 0.0, 0.0] for x in range(0, 6)]
            ...
            try:
                if _grant.clr_prediction_curve[0][1]:
                    ...
                    max_twitter_followers = max(
                        _grant.twitter_handle_1_follower_count, _grant.twitter_handle_2_follower_count)
                    if max_twitter_followers:
                        Stat.objects.create(
                            created_on=from_date,
                            key=_grant.title[0:43] + "_admt1",
                            val=int(
                                100 * _grant.clr_prediction_curve[0][1]/max_twitter_followers),
                        )
        ...
        debug_output.append({'grant': grant.id, "clr_prediction_curve": (
            potential_donations, potential_clr), "grants_clr": grants_clr})
